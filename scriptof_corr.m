...
for k = 0:num_images
    ...
    if k == 0
        ...
        %position à vérifier dans le code
        Zest = mu;
        Yest = Sigma;
        ...
        S = zeros(2 * size(U_pred, 1));

        for i = 1:size(image(2, :), 2)
        ...
        R = eye(size(H, 1)); % Measurement noise regularized

        % Debugging: Check dimensions of matrices
        disp('Dimensions of Yest:');
        disp(size(Yest));
